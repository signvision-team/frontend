...
import { Search, Accessibility, CheckCircle, XCircle, Edit, Award, BookOpen } from 'lucide-react';
...
export function UsersManagement() {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const matchesLevel =
      filterLevel === 'all' ||
      user.learningLevel.toLowerCase() === filterLevel.toLowerCase();
  ...
  const getLevelBadgeClass = (level) => {
    switch(level) {
      ...
      case 'Advanced': return 'bg-yellow-600/20 text-yellow-400';
    ...
    <div className="admin-panel-wrapper">
      <div className="mb-8">
        ...
        <p className="admin-subtitle">
          Monitor student progress and manage specialized learning requirements
        </p>
      </div>
          ...
          <table className="w-full admin-table">
            ...
            <tbody>
              {filteredUsers.map((user) => (
                <tr key={user.id} className="border-b border-gray-700/20 hover:bg-white/5">
                  ...
                  <td className="py-4 px-4">
                    <span className={`px-3 py-1 rounded-full text-xs ${getLevelBadgeClass(user.learningLevel)}`}>
                      {user.learningLevel}
                    </span>
                  </td>
                  ...
                  <td className="py-4 px-4 flex gap-3">
                    <Edit size={18} className="text-gray-400 hover:text-cyan-400 cursor-pointer" />
                    <Award size={18} className="text-gray-400 hover:text-yellow-500 cursor-pointer" />
                  </td>
